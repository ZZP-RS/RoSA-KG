...
def _generate_candi_kg(pmi_dict,n_items, kg_dict, item_embeds=[],pmi_threshold = 0.5, cos_threshold = 0.9):
    ...
    valid_pairs = 0
    for item_pair in tqdm(pmi_dict.keys(), total=len(pmi_dict), desc="[Phase] PMI筛选"):
        item_i, item_j = [int(x) for x in item_pair.split(",")]
        npmi = pmi_dict[item_pair][1]
        if npmi < pmi_threshold:
            continue
        valid_pairs += 1
        item_neb_dict[item_i].add(item_j)
        item_neb_dict[item_j].add(item_i)
    print("pmi num pair: ", valid_pairs)
    
    new_kg_dict = defaultdict(list)
    for k,v in tqdm(item_neb_dict.items(), total=len(item_neb_dict), desc="[Phase] 生成候选KG"):
        temp = []
        for item in v:
            kg_seq = kg_dict[item]
            for r_t in kg_seq:
...
def _cal_npmi(train_user_set):
    item_freq = {}
    for user, item_seq in tqdm(train_user_set.items(), total=len(train_user_set), desc="[Phase] 统计item频次"):
        appeared = set()
    ...
    item_pair_count = {}
    for user, item_seq in tqdm(train_user_set.items(), total=len(train_user_set), desc="[Phase] 计数item对"):
        appeared = set()
    ...
    for item_pair in tqdm(item_pair_count, total=len(item_pair_count), desc="[Phase] 计算NPMI"):
        item_i, item_j = [int(x) for x in item_pair.split(",")]
...
def early_stopping(log_value, best_value, stopping_step, expected_order='acc', flag_step=20):
